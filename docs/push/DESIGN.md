...
> **状态：已裁撤（2026-08-28，上级令 01M13CBY / alice 执行令 01M13CED0）。**
> UI 面已于 v0.6.32 清理车整体移除（设置页推送卡片/SW 订阅链/i18n push.*）；服务端端点保留休眠（VAPID 未配置 = 对外零暴露干净 404），不动产。本文留档防误用：机制未在验收真机做最小原型验证即立项（WebView 无 PushManager 为安卓平台限制），教训成立；替代路线（原生薄壳+轮询本地通知）由 alice 另出原型方案后重议，勿据本文动工。

原稿裁定输入：alice 01M11J45M（路径 C 批准+两点决策+安全补两条）、Felix 01M11J05C（三路径初评）、上级令 01M11HYJ。作者：Devi。
