...
(function () {
  ...
  // v0.6.30 M3 (Iris): push notification preferences. Contract (Devi task
  // pack 01M11KKC): GET /api/push/vapid-key -> {public_key}, 404 when the
  // server hasn't opted in — degrade grey, never error. Subscribe posts the
  // PushSubscription JSON (endpoint + keys, idempotent upsert); DELETE
  // ?endpoint= revokes this device. DND is decided server-side from
  // {enabled,start_min,end_min}, minutes since local midnight; the client
  // only mirrors that window through GET/PUT /api/push/settings. The SW
  // file lives at /static/ for now, so notificationclick opens a fresh tab
  // rather than focusing — a root-scope route is Devi's follow-up.
  (function wirePushPrefs() {
    const toggle = $("#push-toggle");
    if (!toggle) return;
    const stateEl = $("#push-state"), pushHint = $("#push-hint");
    const dndRow = $("#push-dnd-row"), dndEnabled = $("#push-dnd-enabled");
    const dndStart = $("#push-dnd-start"), dndEnd = $("#push-dnd-end");
    const dndNow = $("#push-dnd-now");
    let vapidKey = null, reg = null, sub = null;

    function setDisabled(msg) {
      toggle.disabled = true;
      if (msg) pushHint.textContent = msg;
      dndRow.hidden = true;
    }
    function syncToggleUI() {
      toggle.classList.toggle("active", !!sub);
      toggle.textContent = t(sub ? "push.disable" : "push.enable");
      stateEl.textContent = sub ? t("push.stateOn") : "";
      dndRow.hidden = !sub;
    }
    function urlB64ToUint8Array(b64) {
      const pad = "=".repeat((4 - (b64.length % 4)) % 4);
      const raw = atob((b64 + pad).replace(/-/g, "+").replace(/_/g, "/"));
      return Uint8Array.from(raw.split(""), function (c) { return c.charCodeAt(0); });
    }
    function fillDndSelects() {
      if (dndStart.options.length) return;
      for (let m = 0; m < 24 * 60; m += 15) {
        const label = String(Math.floor(m / 60)).padStart(2, "0") + ":" + String(m % 60).padStart(2, "0");
        dndStart.add(new Option(label, String(m)));
        dndEnd.add(new Option(label, String(m)));
      }
    }
    function dndActiveNow(d) {
      const now = new Date();
      const cur = now.getHours() * 60 + now.getMinutes();
      if (!d.enabled || d.start_min === d.end_min) return false;
      return d.start_min < d.end_min
        ? cur >= d.start_min && cur < d.end_min
        : cur >= d.start_min || cur < d.end_min;
    }
    async function syncDnd() {
      try {
        const d = await api("/api/push/settings", { keepSession: true });
    ...
    async function putDnd() {
      const d = { enabled: dndEnabled.checked,
        start_min: parseInt(dndStart.value, 10) || 0,
        end_min: parseInt(dndEnd.value, 10) || 0 };
      try {
        ...
        dndNow.textContent = t("common.error", { msg: e.message });
      }
    }
    dndEnabled.addEventListener("change", putDnd);
    dndStart.addEventListener("change", putDnd);
    dndEnd.addEventListener("change", putDnd);
    toggle.addEventListener("click", async function () {
      ...
      try {
        if (!sub) {
          ...
          sub = null;
          await api("/api/push/subscribe?endpoint=" + encodeURIComponent(ep), { method: "DELETE" });
        }
        pushHint.textContent = "";
      } catch (e) {
        pushHint.textContent = t("common.error", { msg: e.message });
        try { await ensureSub(); } catch (_) {}
      }
      toggle.disabled = false;
      syncToggleUI();
      if (sub) syncDnd();
    });
    (async function pushBoot() {
      if (!("serviceWorker" in navigator) || !("PushManager" in window) || !("Notification" in window)) {
        return setDisabled(t("push.unsupported"));
      }
      try { vapidKey = await api("/api/push/vapid-key", { keepSession: true }); }
      catch (_) { return setDisabled(t("push.serverOff")); }
      try { await ensureSub(); } catch (_) {}
      fillDndSelects();
      syncToggleUI();
      if (sub) syncDnd();
