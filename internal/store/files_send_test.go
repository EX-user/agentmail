...
func TestSendWithAttachments(t *testing.T) {
	...
	res, err := s.SendWithAttachments("a@t", "a", []string{"b@t", "c@t"}, "with file",
...
func TestSendWithAttachmentsOwnership(t *testing.T) {
	...
	if _, err := s.SendWithAttachments("b@t", "b", []string{"c@t"}, "steal", "x", []string{rec.ID}); err == nil || !strings.Contains(err.Error(), "not owned") {
		t.Errorf("attaching another account's file must fail, got %v", err)
	}
	// Unknown file id.
	if _, err := s.SendWithAttachments("a@t", "a", []string{"b@t"}, "x", "x", []string{"NOPE"}); err == nil || !strings.Contains(err.Error(), "not found") {
